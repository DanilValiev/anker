...
namespace App\Modules\EasyAdmin\Infrastructure\EventSubscriber;

use App\Modules\EasyAdmin\Domain\Events\CustomBeforeEntityPersistedEvent;
use App\Modules\EasyAdmin\Domain\Events\CustomBeforeEntityUpdatedEvent;
...
class CustomPropertyFieldSubscriber implements EventSubscriberInterface
{
     ...
     */
    public function catchCustomParamsField(CustomBeforeEntityPersistedEvent|CustomBeforeEntityUpdatedEvent $event): void
    {
        ...
        foreach ($request[$entityName] as $propertyPath => $property) {
            if (is_array($property) && isset($property['isCustomParams']) && $property['isCustomParams']) {
